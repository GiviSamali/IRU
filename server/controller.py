...
def build_memory_block(machine_guid: str | None) -> str:
    """Собрать блок «Память об этом устройстве» для промпта (≤2048 символов)."""
    if not machine_guid:
        return ""

    facts = db.get_pinned_facts(machine_guid)
    commands = db.get_recent_commands(machine_guid, 20)
    ...
    facts_lines = []
    if facts:
        facts_lines.append("Закреплённые факты:")
        for f in facts:
    ...
    def _assemble(cmd_lines: list[str]) -> str:
        parts = ["## Память об этом устройстве", ""]
        if facts_lines:
            parts.extend(facts_lines)
            parts.append("")
        if cmd_lines:
            parts.append("Последние команды (новые сверху):")
...
async def process_nl_command(
    ...
    memory_block = build_memory_block(machine_guid)
        ...
        for iteration in range(MAX_ITERATIONS):
            ...
            for tc in tool_calls:
                ...
                if fn_name == "execute_cmd":
                    ...
                    if not machine_guid:
                        tool_result = {"result": "Не удалось сохранить факт: устройство не идентифицировано"}
                    else:
                        try:
                            fact_id = db.add_fact(
                                machine_guid=machine_guid,
                            ...
                            tool_result = {"result": f"Факт сохранён (id={fact_id})"}
                        except Exception as e:
                    ...
                    if not machine_guid:
                        ...
                        try:
                            ok = db.delete_fact(machine_guid, int(fn_args.get("fact_id", 0)))
