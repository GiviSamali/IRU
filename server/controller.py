...
async def process_nl_command(
    ...
    commands_log = []
    created_task_ids: list[int] = []  # legacy placeholder: non-pipeline mode больше не создаёт plan-задачи

    model = _pick_model(cfg, modes)
    base_model = cfg.get("model", "deepseek-chat")
    print(f"[llm] выбрана модель: {model} (base={base_model}, autonomous={autonomous})")
