...
В проекте есть базовый regression suite на `pytest`.
...
- публичные endpoint’ы `/api/info` и `/api/plans`;
- legacy `X-Token`;
- JWT `login` / `refresh` / `logout`;
- hashing refresh-token’ов;
- dangerous / confirm классификацию команд;
- onboarding-режим без подключённых устройств;
- free / pro / business лимиты;
- download-токены и Unicode filenames;
- базовые модули агента;
- очистку `runtime_state` между тестами.

Ручное тестирование через UI всё ещё обязательно для пользовательских сценариев:

- запустить сервер и агента локально;
- отправить команду в чат;
- проверить выполнение и ответ;
- для режима План — проверить создание шагов и live-прогресс;
- проверить confirm/deny через интерфейс;
- проверить DEV MODE, если менялись тарифы, UI или raw-command логика.

### Чек-лист регрессии перед merge

- Запустить `python -m pytest -q tests`.
- Убедиться, что `git status --short` пустой.
- Не коммитить `llm_config.json`, `iru.db`, `agent/config.json`, `server/.jwt_secret`.
- Убедиться, что тесты не обращаются к реальному DeepSeek API.
- Убедиться, что `runtime_state` очищается между тестами.
- Если менялась авторизация — отдельно проверить legacy `X-Token` и JWT `login` / `refresh` / `logout`.
- Если менялась безопасность команд — отдельно проверить dangerous / confirm сценарии.
- Если менялись тарифы — отдельно проверить free / pro / business лимиты.
